...
## remove all vowels
  def test_remove_vowels
    ...
    letters.each do |letter|
      remaining << letter unless letter.include?("e")
    end
  ...
  def test_remove_words_with_more_than_three_letters
    ...
    selected = []
    words.each {|e| selected << e unless e.length > 3}
  ...
  def test_remove_words_ending_in_e
    ...
    selected = []
    words.each {|e| selected << e unless e.end_with?("e")}
  ...
  def test_remove_words_ending_in_ing
    ...
    selected = []
    words.each {|e| selected << e unless e.end_with?("ing")}
  ...
  def test_remove_words_containing_e
    ...
    selected = []
    words.each {|e| selected << e unless e.include?("e")}
  ...
  def test_remove_dinosaurs
    ...
    notasaurus = []
    animals.each {|e| notasaurus << e unless e.include?("saurus")}
  ...
  def test_remove_numbers
    ...
    not_numbers = []
    elements.each {|e| not_numbers << e if e == e.to_s}
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each {|e| not_numbers << e if e == e.to_s || e == e.to_i }
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each {|e| remaining << e unless e.start_with?("a", "e", "i", "o", "u")}
  ...
  def test_remove_capitalized_words
    ...
    remaining = []
    words.each {|e| remaining << e if e != e.upcase}
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each {|e| remaining << e unless e.class == Array }
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each do |e|
      remaining << e unless e.class == Hash
    end
