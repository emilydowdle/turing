...
class AnyPatternTest < Minitest::Test
  ...
  def test_has_at_least_one_alice
    ...
    has_alice = false
    names.each {|e| has_alice = true unless e != "Alice"}
    assert has_alice
  end
  ...
  def test_has_a_multi_word_phrase
    ...
    phrases.each {|e| has_multi_word_phrase = false unless e.length > 1}
  ...
  def test_no_monkeys
    ...
    has_monkeys = false
    animals.each {|e| has_monkeys = false unless e == "monkey"}
    refute has_monkeys
  end
  ...
  def test_no_multiples_of_five
    ...
    numbers.each {|e| multiples_of_5 = true unless (e * 5) % 5 == 0}
