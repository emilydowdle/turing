...
class HashTest < Minitest::Homework
  ...
  def test_include
    ...
    assert boolean
    boolean = {"a" => "apple"}.include?("apple") # include? only good for keys?
    refute boolean
  end

  def test_member
    boolean = {}.member?("a") # member? same as include?
    refute boolean
  ...
  def test_invert
    assert_equal ({"banana" => "b", "apple" => "a"}), {"a" => "apple", "b" => "banana"}.invert
