...
class ArrayTest < Minitest::Homework
  ...
  def test_take
    assert_equal [], [].take(1)
    assert_equal [], [].take(2)
    assert_equal ["a", "b", "c"], ["a", "b", "c", "d", "e"].take(3) # .take returns the number of elements, starting at [0]
  ...
  def test_minus
    ...
    assert_equal [:b, :c], a3
    assert_equal [:c], a4
    assert_equal [:b, :c], a5 # removes all :a, not just one instances
  end
  ...
  def test_pipe
    ...
    assert_equal [], a1 # single array, not nested ArrayTest
    assert_equal [:a], a2
    assert_equal [:a, :b], a3
    assert_equal [:a, :b, :c], a4 # destroys duplicates
    assert_equal [:a, :b, :c], a5
  end
  ...
  def test_ampersand
    ...
    assert_equal [], a1
    assert_equal [], a2 # blank array?? returns element that's the same in each
    assert_equal [], a3
    assert_equal [:b], a4
    assert_equal [:b], a5
  end
  ...
  def test_push
    ...
    assert_equal [1, "apple", :c, ["fish", 7, :x]], [1, "apple", :c].push(["fish", 7, :x]) # pushes array into array, needs comma after :c
  end

  def test_shift
    assert_equal nil, [].shift # because it returns the removed element
  ...
  def test_parallel_assignment
    ...
    *a, b = fruit
    assert_equal ["apple", "banana", "cherry"], a # why not dewberry? because splat leaves room for the number of elements listed beyond?
  ...
  def test_delete
    ...
    assert_equal ["chicken", "steak"], protein # deletes all instances of bacon
  end
  ...
  def test_compact
    ...
    assert_equal ["a", "b", "c"], ["a", nil, "b", "c", nil, nil].compact # just removes nil?
